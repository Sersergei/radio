<?php
return array(
    'What do you think about this song?' => 'Что Вы думаете об этой песне?',
    'If you wanna listen this song again, click the button please!' => 'Если хотите прослушать эту песню еще раз, нажмите пожалуйста на кнопку',
    'I never heard of it' => 'Я никогда не слышал эту песню',
    'Even if you don`t know the song, do you like it?' => 'Даже если Вы раньше не слышали эту песню, она Вам нравится?',
    'I love it' => 'Эту песню я обожаю',
    'I just like it'=>'Что-то в этой песне мне нравится',
    'I would listen to it'=>'Эта песня безразлична мне',
    'I`m tired of it'=>'Когда-то эта песня мне нравилась',
    'I don`t like it'=>'Эта песня мне не нравится',
    'listen a previous song again'=>'прослушать предыдущую песню еще раз',
    'Choose one music-mix, which you like more than other'=>'Выберите один из миксов, который вам нравится больше других и нажмите кнопку "Вперёд"',
    'Next'=>'Дальше',
    'Create Users'=>'Регистрация',
    'Registration'=>'Регистрация',
    'Fields with* are required.'=>'Заполните все поля',
    'ID-card'=>'карта',
    'Your Name'=>'Ваше имя',
    'Birth Date'=>'Дата рождения',
    'Sex'=>'Пол',
    'Where are you from?'=>'Откуда Вы?',
    'What education do you have?'=>'Какое у Вас образование?',
    'What radiostation are you listen more than other on last week?'=>'Выберите радиостанцию, которую вы слушали чаще других в течение последней недели',
    'What other radiostations are you listen yet on last week?'=>'Выберите еще одну радиостанцию, которую вы слушали немного реже, чем любимую станцию в течение последней недели',
    'Create'=>'Завершить',
    'We must choose one mix'=>'Вы должны выбрать один микс',
    'your name' => 'имя',
    'your birth date' => 'дата рождения',
    'your e-mail' => 'email',
    'your sex' => 'пол',    
    'your adress' => 'регион',
    'yours education' => 'образование',
    'Please fix errors'=>'Необходимо исправить ошибку',
    'enter your name'=>'введите имя',
    'enter your Date Birth'=>'введите дату своего рождения в формате: год, месяц, число',
    'enter your sex'=>'укажите свой пол',
    'enter your region'=>'укажите регион своего проживания',
    'enter your education'=>'укажите свое образование',
    'enter your email'=>'укажите Ваш email',
    'You already chose this radiostation'=>'Вы уже выбрали эту радиостанцию',
    'You entered the incorrect date'=>'введите дату своего рождения в формате: год, месяц, число',
    'You chosen this radio station. You must choose other radio, which listened less, than your favorite station'=>'Вы уже выбрали эту радиостанцию, теперь выберите другую станцию, которую вы слушали немного реже, чем любимую станцию',
    'Thank you! The invitation to test you come to the email'=>'Спасибо за регистрацию! Приглашение на тестирование музыки придет Вам на email',
    'For beginning testing music you must click this '=>'Для того, чтобы начать тестирование музыки, вам необходимо нажать на эту ',
    'link'=>'ссылку',
    'Next'=>'Вперёд',
    'I will now play songs to you. Check now, that your speakers are turned on and the volume is not too loud. After listening to each song, I will ask you a few questions.Thank you for your time!'=>'Через пару секунд я начину проигрывать вам фрагменты песен. Проверьте играет ли сейчас музыка в колонках или наушниках, а также проверьте уровень громкости. Не слишком тихо или громко? После прослушивания каждой песни, я задам вам несколько вопросов. Спасибо за ваше время!',
    'Login with Facebook'=>'Войти через Facebook',
    'Man'=>'мужской',
    'Woman'=>'женский',
    'pupil of sсhool'=>'учусь в школе',
    'student'=>'студент(ка)',
    'secondary education'=>'среднее образование',
    'higher education'=>'высшее образование',
    'YYYY-MM-DD'=>'(год-месяц-дата)',
    'click here'=>'нажмите тут',
    'click here.'=>'нажмите тут.',
    'Unscribe'=>'Отписаться',
    'Thank you for your request to be added to the web Radio Music Test We add names to our list only after verifying the recipient`s permission, which is why we are sending you this address confirmation request To confirm that you would like to receive our future invite on the music tests, please '=>'cпасибо за ваш запрос в Radio Music Test. Мы добавляем имена в наш список только после проверки разрешения получателя, поэтому мы посылаем вам этот запрос подтверждения емейла. После подтверждения, вы будете получать приглашения на тестирование музыки, пожалуйста ',
    'To complete the registration click on the link in the email'=>'Для завершения регистрации перейдите по ссылке, отправленной на адрес электронной почты. Если в течение одной минуты вы не получили письмо с подтверждением, проверьте, не попало ли оно в спам.',
    'For beginning testing music you must click this link'=>'Для того, чтобы начать тестирование музыки, перейдите по этой ссылке ',
    'Unscribe'=>'Отписаться',
    'periodicity'=>'Как часто вы хотели бы получать приглашения для тестирования музыки?',
    'once a week'=>'раз в неделю',
    'two times a month'=>'раз в две недели',
    'once a month'=>'раз в месяц',
    'Message'=>'Тут вы можете написать нам пожелание и предложения по музыке в эфире',
    'Send'=>'Отправить',
    'Thank you Your message has been sent'=>'Спасибо! Ваше сообщение было отправлено.',
    'Sorry you can not send a message'=>'К сожалению ваше сообщение не было отправлено.',
    'If you wanna change settings your profile'=>'Если вы хотите изменить данные в своей анкете или реже получать приглашения для тестирования музыки, ',
    'VIGA: Isikukood on vigane!'=>'Персональный код ID-карты неверен!',
);

